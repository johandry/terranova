...
func (p *Platformer) State() *terraform.State {
	if p.state == nil {
...
func (p *Platformer) WriteState(w io.Writer) error {
...
func (p *Platformer) ReadState(r io.Reader) error {
...
func (p *Platformer) WriteStateFile(filename string) error {
...
func (p *Platformer) ExistStateFile(filename string) bool {
...
func (p *Platformer) ReadStateFile(filename string) ([]byte, error) {
...
func (p *Platformer) WriteStateBuf(state []byte) error {
...
func (p *Platformer) ReadStateBuf(state []byte) error {
	if len(state) > 0 {
		buf := bytes.NewBuffer(state)
		tfState, err := terraform.ReadState(buf)
		if err != nil {
